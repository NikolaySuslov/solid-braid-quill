...
let braid_text = {
    verbose: false,
    db_folder: './src/lib/braid-text/braid-text-db',
...
braid_text.serve = async (req, res, options = {}) => {
    ...
    if (req.method == "GET" || req.method == "HEAD") {
        if (!req.subscribe) {
            ...
            res.setHeader("Repr-Digest", `sha-256=:${crypto.createHash('sha256').update(buffer).digest('base64')}:`)
...
braid_text.list = async () => {
    try {
        if (braid_text.db_folder) {
            ...
            var pages = new Set()
            for (let x of await fs.promises.readdir(braid_text.db_folder)) pages.add(decode_filename(x.replace(/\.\w+$/, '')))
            return [...pages.keys()]
...
function validate_version_array(x) {
    ...
    x.sort()
    for (const xx of x) validate_actor_seq(xx)
}
